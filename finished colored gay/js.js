...
document.addEventListener('wheel', (e) => {
    ...
    count += e.deltaY > 0 ? 1 : -1;

    if (Math.abs(count) >= 5) {
        changeBox(count > 0 ? 1 : -1);
        count = 0;
    }
}, { passive: false });
...
function updateNavItems() {
  document.querySelectorAll('.nav-item[data-box]').forEach(item => {
      const boxIndex = parseInt(item.dataset.box);
      item.classList.toggle('active', boxIndex === index);
  });
}
...
document.querySelectorAll('.nav-item[data-box]').forEach(item => {
  item.addEventListener('click', (e) => {
      e.preventDefault();
      index = parseInt(item.dataset.box);
      boxes.forEach((box, i) => {
          box.classList.toggle('active', i === index);
      });
      updateNavItems();
